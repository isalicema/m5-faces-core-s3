...
namespace suite {
enum class App : uint32_t { Home=0, Music=1, Radio=2, Companion=3, Connection=4 };
...
inline App consume(Ticket& t,bool softwareReset){
    Ticket copy=t;t={};
    if(!softwareReset||copy.magic!=marker||copy.check!=(marker^copy.app^0xA55A)||copy.app>4)return App::Home;
    return App(copy.app);
}
